...
```{r}
library(DT)
diff_table_reg <- subset(diff_table, mode == "sig")
datatable(diff_table_reg, options = list(pageLength = 10))
```

## Table under "Reg" Mode
```{r}
diff_table_sig <- subset(diff_table, mode == "reg")
...
```{r}
pheno_path <- "/dartfs/rc/lab/S/Szhao/qiruiz/diffdriver/batch3/tumor_specific_input/KICH/KICH_Tem_cells.txt"
pheno <- read.table(pheno_path, header = TRUE, sep = "\t")
head(pheno)
```

```{r}
diffdriver::plot_mut(gene_name = "TP53", mut= mut, pheno = pheno, totalnttype = 96, 
