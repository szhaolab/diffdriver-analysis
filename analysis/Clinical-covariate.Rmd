...
In total, **160 analyses** were attempted across **20 tumor types**, involving the following **8 distinct phenotypes**:
...
For all phenotypes **except Diagnosis Age**, both **age** and **gender** were included as covariates. When **Diagnosis Age** itself was used as the phenotype, only **gender** was included as a covariate.

For each phenotype, the analyses for **CESC, PRAD, TGCT, UCEC, and UCS** failed because the gender covariate had no variation within these tumor types: **CESC = all female, PRAD = all male, TGCT = all male, UCEC = all female, and UCS = all female**. Therefore, **120 analyses were successfully completed**, while **40 analyses failed due to the single-level gender covariate**.
